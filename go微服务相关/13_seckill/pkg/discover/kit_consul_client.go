...
func New(consulHost, consulPort string) *DiscoverClientInstance {
	...
	// 通过host和post创建一个consulclient
	cosulConfig := api.DefaultConfig()
	cosulConfig.Address = consulHost + ":" + strconv.Itoa(port)
	apiclient, err := api.NewClient(cosulConfig)
	if err != nil {
		log.Fatal(err)
		return nil
	}
...
func (consulClient *DiscoverClientInstance) Register(instanceId, svcHost, healthCheckUrl, svcPort string, svcName string, weight int, meta map[string]string, tags []string, logger *log.Logger) bool {
